...
import type { InputObject, LucidFilterContract } from "./types/filter.ts";
import type { ModelQueryBuilderContract, QueryScope, QueryScopeCallback } from "@adonisjs/lucid/types/model";

export const Filterable = <Model extends NormalizeConstructor<typeof BaseModel>>(superclass: Model) => {
  class FilterableModel extends superclass {
     ...
     */
    static filter<T extends typeof FilterableModel, Filter extends LucidFilterContract = ReturnType<T["$filter"]>>(
      this: T,
     ...
     */
    static filtration = function (this: typeof FilterableModel, query, input, filter?: LucidFilterContract) {
      const Filter = filter || this.$filter();
